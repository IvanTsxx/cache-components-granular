...
export function AISearchPanelHeader({
  ...
  const { setOpen, pageName } = useAISearchContext();
    ...
    >
      <div className="px-3 py-2 flex-1">
        ...
        <p className="text-xs text-fd-muted-foreground mb-2">
          {lang === "es"
            ? "Esta IA solo responderá preguntas sobre esta página:"
            : "This AI will answer questions only about this page:"}
        </p>

        <p className="text-xs font-semibold text-fd-primary">{pageName}</p>
      </div>
...
export function AISearchInput(props: ComponentProps<"form">) {
  ...
  const isLoadingText =
    lang === "es" ? "La IA está respondiendo..." : "AI is answering...";
  const placeholderText = lang === "es" ? "Haz una pregunta" : "Ask a question";
  ...
  return (
    ...
    >
      <Input
        value={input}
        placeholder={isLoading ? isLoadingText : placeholderText}
        autoFocus
        className="p-3"
      ...
      {isLoading ? (
        ...
        >
          ...
          {lang === "es" ? "Abortar respuesta" : "Abort Answer"}
        </button>
      ) : (
        <button
          key="bn"
...
function Message({
  ...
  for (const part of message.parts ?? []) {
    ...
    if (part.type === "tool-provideLinks" && part.input) {
      ({ links } = part.input as z.infer<typeof ProvideLinksToolSchema>);
    }
  }
          ...
          {links.map((item, i) => (
            ...
            >
              ...
              <p className="text-fd-muted-foreground">
                {lang === "es" ? "Referencia" : "Reference"} {item.label}
              </p>
            </Link>
          ))}
        </div>
...
export function AISearch({ children }: { children: ReactNode }) {
  ...
  const chat = useChat({
    ...
    transport: new DefaultChatTransport({
      api: "/api/chat",
      headers: {
        "x-current-lang": lang as string,
        "x-current-page": pathname.split("/").pop() || "",
      },
    }),
  });
  ...
  return (
    <Context
      value={useMemo(
        () => ({ chat, lang: lang as string, open, pageName, setOpen }),
        [chat, open, pageName, lang as string]
      )}
    >
      {children}
    </Context>
