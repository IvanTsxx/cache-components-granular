...
<Tabs items={['revalidateTag', 'updateTag']}>
  <Tab value="revalidateTag">
    ...
    <Mermaid chart={`sequenceDiagram
        ...
        Note over S: Cache (Válido)
        U1->>S: Click "Revalidar"
        Note over S: Cache marcado STALE
        S-->>U1: Sirve versión vieja
    
        U2->>S: Visita página
        S-->>U2: Sirve versión vieja
        Note over S: Trigger regeneración en background
        Note over S: Regeneración completa
        Note over S: Cache (Válido)
    
        U3->>S: Visita página
        S-->>U3: Sirve versión NUEVA ✨`} />
  ...
  <Tab value="updateTag">
    ...
    <Mermaid chart={`sequenceDiagram
        ...
        Note over S: Cache (Válido)
        U1->>S: Click "Agregar al carrito"
        Note over S: Cache EXPIRA inmediatamente
        
        U2->>S: Visita página
        Note over S: Regenera en request
        S-->>U2: Sirve versión NUEVA ✨
        Note over S: Cache (Válido)`} />
