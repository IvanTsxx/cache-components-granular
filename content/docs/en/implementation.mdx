...
Without `generateStaticParams`, the dynamic route still works, but you lose part of the educational value of clearly seeing which paths are prerendered at build time.
</Callout>
...
<Callout type="warning">
**Why multiple queries?** Although it seems inefficient, the cached queries are executed at **build time**, not on every request.
</Callout>
...
**No `use cache`** = the component executes on every request and streams to the client.
</Callout>
...
<Callout type="error">
**Critical:** `params` is runtime data and must be accessed inside a Suspense boundary when Partial Prerendering is enabled.
</Callout>
...
<Callout type="info">
During **Build Time**, text and price execute their DB queries and the results get saved in the prerender. The stock component is ignored. <br/>
During **Request Time**, the stock executes its DB query while the user is already seeing the prerendered content (static shell).
</Callout>
...
<Callout type="success">
If you see a `◐ Partial Prerender` indicator for `/product/[id]`, it is working correctly for this Cache Components demo.
</Callout>
